...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        if let window = self.window {
            ...
            AppStoreHolder.store.subscribe(controlPanel!);
        }

        // Create player and subscirbe it
        appAudioPlayer = AppAudioPlayer();
        AppStoreHolder.store.subscribe(appAudioPlayer!);

        // Init log console
        AEConsole.launch(with: self);
        // Restore preferences see at reducer
        AppStoreHolder.store.dispatch(AppAction.RestorePreferences);
